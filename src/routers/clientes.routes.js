...
router.post('/clienteAct', async (req, res) => {
    ...
    for (const actividad of actividades) {
        ...
        const cantidadVehiculosNecesarios = Math.ceil(actividad.cantidadMaterial / 7); // Ejemplo: 14 -> 2 vehículos
        ...
        try {
            ...
            const { actividad: updatedActividad, registrosHistorial } = await asignarVehiculosYCalcularHoras(actividadExistente);
            ...
            return res.status(500).json({ error: "Ocurrió un error al intentar crear la actividad. Por favor, intente de nuevo más tarde." });
        }
    }
