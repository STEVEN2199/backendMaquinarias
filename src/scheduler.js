...
export const asignarVehiculosYCalcularHoras = async (actividad) => {
    try {
        ...
        const actividadesExistentesResponse = await axios.get('http://localhost:3000/api/actividadesPorFechaHora', {
            params: {
        ...
        if (actividadesExistentes.length > 0) {
            const maquinariasPorFechaResponse = await axios.get('http://localhost:3000/api/maquinariasPorFecha', {
                params: {
                    fecha: horaInicioTrabajo.toISOString().split('T')[0]
                }
            });
            ...
            if (maquinariasPorFecha.length === 0) {
                const maquinariasTotalResponse = await axios.get('http://localhost:3000/api/maquinariasTotal');
                maquinariasDisponibles = maquinariasTotalResponse.data.filter(maquinaria => 
                    maquinaria.estado === 'activo' && (!maquinaria.horaDisponible || new Date(maquinaria.horaDisponible) <= horaInicioTrabajo)
                );
            } else {
                maquinariasDisponibles = maquinariasPorFecha.filter(maquinaria => 
                    maquinaria.estado === 'activo' && (!maquinaria.horaDisponible || new Date(maquinaria.horaDisponible) <= horaInicioTrabajo)
                );
            }
        } else {
        ...
        if (maquinariasDisponibles.length < cantidadVehiculos) {
            console.log(`Error: No hay suficientes maquinarias disponibles. Se necesitan ${cantidadVehiculos}, pero solo hay ${maquinariasDisponibles.length}.`);
            throw new Error('No hay suficientes maquinarias disponibles para esta actividad.');
        }
        ...
        for (let i = 0; i < cantidadVehiculos; i++) {
            ...
            const horaSalida = new Date(horaInicioTrabajo.getTime() - tiempoViaje * 60000);
            console.log("Hora salida:", horaSalida);
            const horaDespacho = new Date(horaInicioTrabajo.getTime() + tiempoDespacho * 60000);
            console.log("Hora despacho:", horaDespacho);
            const horaFinTrabajo = new Date(horaDespacho.getTime() + (TIEMPO_LAVADO + tiempoViaje + TIEMPO_EXTRA) * 60000);
        ...
        if (registrosHistorial.length > 0) {
            const horaFinActividad = registrosHistorial[registrosHistorial.length - 1].horaFinTrabajo;
